...
short_description: Mounts Filesystems of Cloud Images using guestfish Python API
...
options:
    ...
    force:
        description:
            - Force the creation of the filesystem regardless if the device in initialized
        required: false
    state:
       description:
...
def run_module():
    ...
    try:
        ...
        if state == "present":
            gfs_result = gfs_util.create_fs(device, device_type, fstype, force, use_image_mkfs_xfs)
        elif state == "absent":
            # Remove Filesystem
